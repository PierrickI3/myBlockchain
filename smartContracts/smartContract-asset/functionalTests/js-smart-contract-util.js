...
class SmartContractUtil {
    ...
    static async getConnectionProfile() {
        ...
        const connectionProfilePath = path.join(homedir, '.fabric-vscode', 'v2', 'environments', 'ccTickets', 'gateways', 'Org1 Gateway.json');
    ...
    static async submitTransaction(contractName, functionName, args, gateway) {
        ...
        let contract;
        if (contractName !== '') {
            contract = await network.getContract('MyAsset', contractName);
        } else {
            contract = await network.getContract('MyAsset');
        }
